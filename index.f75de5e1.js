...
tableHeaders.forEach((header, index)=>{
    header.addEventListener("click", ()=>{
        ...
        const sortedRows = [
            ...
            if (currentSort.asc) return isNaN(numA) || isNaN(numB) ? cellA.localeCompare(cellB) : numA - numB;
            else return isNaN(numA) || isNaN(numB) ? cellB.localeCompare(cellA) : numB - numA;
        });
...
form.addEventListener("submit", (e)=>{
    ...
    if (nameValue.length < 4 || ageValue < 18 || ageValue > 90 || isNaN(Number(salaryValue))) {
        ...
        notification.setAttribute("data-qa", "notification");
        notification.textContent = "New employee is successfully added to the table";
        document.body.appendChild(notification);
        setTimeout(()=>{
            document.body.removeChild(notification);
        }, 5000);
